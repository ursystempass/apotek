                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('nonota_jual') is-invalid @enderror"
                                ...
                                value="{{ old('nonota_jual', $penjualan->nonota_jual) }}">
                            @error('nonota_jual')
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control @error('tgl_jual') is-invalid @enderror"
                                id="exampleInputTanggalJual" name="tgl_jual" value="{{ old('tgl_jual', $penjualan->tgl_jual ? \Carbon\Carbon::parse($penjualan->tgl_jual)->format('Y-m-d\TH:i') : '') }}"
                            @error('tgl_jual')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('total_jual') is-invalid @enderror"
                                ...
                                value="{{ old('total_jual', $penjualan->total_jual) }}">
                            @error('total_jual')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control @error('user_id') is-invalid @enderror" id="exampleInputUser"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $user->id == $penjualan->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                            @error('user_id')
                                <span class="text-danger">{{ $message }}</span>
